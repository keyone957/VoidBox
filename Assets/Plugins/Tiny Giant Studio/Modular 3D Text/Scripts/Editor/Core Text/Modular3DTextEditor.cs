...
namespace TinyGiantStudio.Text
{
    ...
    public class Modular3DTextEditor : Editor
    {
        ...
        private SerializedProperty autoFontSize;
        ...
        private SerializedProperty sizeGraceAmount;
        private SerializedProperty fontSizeIterationAmount;
        private SerializedProperty maxIterationTimes;
        ...
        private SerializedProperty characterObjectList;
        ...
        private void MainSettings()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(showMainSettingsInEditor.faded))
            {
                ...
                {
                    if ((myTarget.GetComponent<LinearLayoutGroup>() || myTarget.GetComponent<GridLayoutGroup>()))
                    {
                        GUILayout.Space(5);

                        if (SupportsAutoFontSize())
                            MText_Editor_Methods.ItalicHorizontalField(autoFontSize, "Auto Font Size", "For grid layout, this can be an expensive process. Please use this feature sparingly. Auto size on Linear layout is a much faster process." + variableName + "autoFontSize", FieldSize.large);

                        if (myTarget.autoFontSize)
                        {
                            MText_Editor_Methods.ItalicHorizontalField(minFontSize, "Minimum Font Size", "" + variableName + "minFontSize", FieldSize.large);
                            MText_Editor_Methods.ItalicHorizontalField(maxFontSize, "Maximum Font Size", "" + variableName + "maxFontSize", FieldSize.large);
                            MText_Editor_Methods.DisabledItalicHorizontalField(fontSize, "Current Font Size", "Controlled by Auto Font Size" + variableName + "FontSize", FieldSize.large);
                            GUILayout.Space(5);
                        }
                        else
                            MText_Editor_Methods.ItalicHorizontalField(fontSize, "Font Size", "Assigning a new font size recreates the entire text. This is to avoid interfering with anything any module or usercreated code is doing." + variableName + "FontSize", FieldSize.normal);
                    }
                    else
                        MText_Editor_Methods.ItalicHorizontalField(fontSize, "Font Size", "Assigning a new font size recreates the entire text. This is to avoid interfering with anything any module or usercreated code is doing." + variableName + "FontSize", FieldSize.normal);
                }
        ...
        private bool SupportsAutoFontSize()
        {
            if (myTarget.GetComponent<LinearLayoutGroup>())
                if (myTarget.GetComponent<LinearLayoutGroup>().alignment != LinearLayoutGroup.Alignment.HorizontalMiddle && myTarget.GetComponent<LinearLayoutGroup>().alignment != LinearLayoutGroup.Alignment.Left && myTarget.GetComponent<LinearLayoutGroup>().alignment != LinearLayoutGroup.Alignment.Right)
                    return false;
            return true;
        }

        /// <summary>
        /// For performance, split the layout settings to two methods.
        ...
        private void AdvancedSettings()
        {
            ...
            {
                if (myTarget.GetComponent<GridLayoutGroup>() != null)
                {
                    if (myTarget.GetComponent<Modular3DText>().autoFontSize)
                    {
                        EditorGUILayout.BeginVertical("Box");
                        EditorGUI.indentLevel = 0;
                        EditorGUILayout.LabelField("Auto font size settings", EditorStyles.boldLabel);

                        MText_Editor_Methods.ItalicHorizontalField(sizeGraceAmount, "Size Grace Amount", "This amount of size difference is ignored when using auto size.", FieldSize.gigantic);
                        MText_Editor_Methods.ItalicHorizontalField(fontSizeIterationAmount, "Font Size Iteration Amount", "This is the value by which font size is increased/decreased iteratively to check if it fits.", FieldSize.gigantic);
                        MText_Editor_Methods.ItalicHorizontalField(maxIterationTimes, "Max Iteration Times", "The maximum amount of times this process is repeated before the layout system gives up trying to fit the text.", FieldSize.gigantic);
        ...
        private void DebugView()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(showDebugSettingsInEditor.faded))
            {
                ...
                EditorGUILayout.PropertyField(characterObjectList);
            }
        ...
        private void FindProperties()
        {
            ...
            autoFontSize = soTarget.FindProperty("autoFontSize");
            minFontSize = soTarget.FindProperty("minFontSize");
            maxFontSize = soTarget.FindProperty("maxFontSize");

            _wordSpacing = soTarget.FindProperty("_wordSpacing");
            autoLetterSize = soTarget.FindProperty("_autoLetterSize");
            sizeGraceAmount = soTarget.FindProperty("sizeGraceAmount");
            fontSizeIterationAmount = soTarget.FindProperty("fontSizeIterationAmount");
            maxIterationTimes = soTarget.FindProperty("maxIterationTimes");
            ...
            characterObjectList = soTarget.FindProperty("characterObjectList");
