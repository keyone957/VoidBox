    ...
    public class GetCharacterObject
    {
        ...
        private static MeshLayout ProcessNormalCharacter(char c, Modular3DText text, MeshPostProcess meshPostProcess)
        {
            ...
            else
            {
                ...
                if (mesh == null || text.Font.TypeFace.glyphs[c].GlyphIndex == 0)
                {
                    mesh = GetMeshFromFallbackFonts(c, text, creator, mesh);
                }
        ...
        {
            for (int i = 0; i < text.Font.fallbackFonts.Count; i++)
            {
                if (text.Font.fallbackFonts[i] == null) continue;

                var tryMesh = creator.GetMesh(
                    text.Font.fallbackFonts[i].GetInstanceID(),
                    text.Font.fallbackFonts[i].TypeFace,
                    text.Font.sizeXYInput, text.Font.sizeZInput,
                    text.Font.fallbackFonts[i].autoSmoothAngleInput,
                    text.Font.fallbackFonts[i].averageYValue, c);

                if (tryMesh != null)
                {
                    if (text.Font.fallbackFonts[i].TypeFace.glyphs[c].GlyphIndex != 0)
                    {
                        text.DestroyMesh(mesh);
        ...
        private static MeshLayout ProcessNormalCharacter(char previousChar, char currentChar, Modular3DText text, MeshPostProcess meshPostProcess)
        {
            ...
            else
            {
                ...
                if (mesh == null || text.Font.TypeFace.glyphs[currentChar].GlyphIndex == 0)
                {
                    mesh = GetMeshFromFallbackFonts(currentChar, text, creator, mesh);
                }
