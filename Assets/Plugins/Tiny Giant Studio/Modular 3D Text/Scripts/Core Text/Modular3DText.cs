...
namespace TinyGiantStudio.Text
{
    ...
    public class Modular3DText : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            ...
            if (GetComponent<MeshFilter>() != null)
                if (GetComponent<MeshFilter>().sharedMesh)
                    DestroyMesh(GetComponent<MeshFilter>().sharedMesh);
        ...
        public void UpdateText()
        {
            ...
            if (createChilds)
            {
                ...
                else
                {
                    ...
                    if (!autoFontSize)
                    {
                        if (!TryGetComponent<GridLayout>(out var gridLayout))
                        {
                            if (!TryGetComponent<LinearLayoutGroup>(out var linearLayoutGroup))
                            {
                                autoFontSize = false;
                            }
                        }
                    }
                    //if (autoFontSize) startCreatingCharacterFromIndex = 0;
                }
            }
            ...
            else
            {
                ...
                UpdateChildrenPositions(startCreatingCharacterFromIndex);
        ...
        private void UpdateChildrenPositions(int startCreatingCharacterFromIndex)
        {
            if (GetComponent<LayoutGroup>())
            {
                if (repositionOldCharacters)
                {
                    if (autoFontSize)
                    {
                        ...
                        {
                            if (
                                linearLayoutGroup.alignment == LinearLayoutGroup.Alignment.HorizontalMiddle
                                || linearLayoutGroup.alignment == LinearLayoutGroup.Alignment.Left
                                || linearLayoutGroup.alignment == LinearLayoutGroup.Alignment.Right
                                )
                            {
                                linearLayoutGroup.UpdateLayout(true);

                                float multiplier = linearLayoutGroup.width / linearLayoutGroup.totalSpaceTaken;
                                var correctedFontSize = GetChildSize(multiplier);

                                float widthChangeRate = correctedFontSize.x / FontSize.x;
                                float heightChangeRate = correctedFontSize.y / FontSize.y;
                                _fontSize = correctedFontSize;

                                //Modify the scale first.
                                for (int i = 0; i < transform.childCount; i++)
                                {
                                    Vector3 size = transform.GetChild(i).localScale;
                                    size.x *= widthChangeRate;
                                    size.y *= heightChangeRate;
                                    transform.GetChild(i).localScale = size;
                                }

                                for (int i = 0; i < linearLayoutGroup.bounds.Length; i++)
                                {
                                    Vector3 size = linearLayoutGroup.bounds[i].size;
                                    size.x *= widthChangeRate;
                                    //size.y *= heightChangeRate;
                                    linearLayoutGroup.bounds[i].size = size;
                                }
                                linearLayoutGroup.UpdateLayoutDoNotUpdateBounds();
                            }
                            else
                            {
                                linearLayoutGroup.UpdateLayout(0);
                            }
                        }
                        else
                        if (TryGetComponent<GridLayoutGroup>(out var gridLayoutGroup))
                        {
                            gridLayoutGroup.UpdateLayout();

                            float totalVerticalSpaceTaken = gridLayoutGroup.totalVerticalSpaceTaken;

                            bool tooBig = false;
                            bool tooSmall = false;
                            if (totalVerticalSpaceTaken > (gridLayoutGroup.Height + sizeGraceAmount))
                                tooBig = true;
                            else if (totalVerticalSpaceTaken < (gridLayoutGroup.Height - sizeGraceAmount))
                                tooSmall = true;

                            Vector3 newFontSize;
                            float widthChangeRate;
                            float heightChangeRate;
                            int loopBreaker = 0;
                            while (tooBig && loopBreaker < maxIterationTimes)
                            {
                                loopBreaker++;
                                newFontSize = new Vector3(
                                        Mathf.Clamp(FontSize.x - fontSizeIterationAmount, minFontSize.x, maxFontSize.x),
                                        Mathf.Clamp(FontSize.y - fontSizeIterationAmount, minFontSize.y, maxFontSize.y),
                                        Mathf.Clamp(FontSize.z, minFontSize.z, maxFontSize.z));

                                widthChangeRate = newFontSize.x / FontSize.x;
                                heightChangeRate = newFontSize.y / FontSize.y;
                                ...
                                foreach (Transform child in transform)
                                {
                                    ...
                                    {
                                        layoutElement.width *= widthChangeRate;
                                        layoutElement.height *= heightChangeRate;
                                    }
                                }
                                //for (int i = 0; i < transform.childCount; i++)
                                //{
                                ...
                                for (int i = 0; i < gridLayoutGroup.bounds.Length; i++)
                                {
                                    Vector3 size = gridLayoutGroup.bounds[i].size;
                                    size.x *= widthChangeRate;
                                    size.y *= heightChangeRate;
                                    gridLayoutGroup.bounds[i].size = size;
                                }
                                _fontSize = newFontSize;

                                gridLayoutGroup.UpdateLayout(0, false);
                                totalVerticalSpaceTaken = gridLayoutGroup.totalVerticalSpaceTaken;
                            ...
                            while (tooSmall && loopBreaker < maxIterationTimes)
                            {
                                loopBreaker++;
                                newFontSize = new Vector3(
                                        Mathf.Clamp(FontSize.x + fontSizeIterationAmount, minFontSize.x, maxFontSize.x),
        ...
        public Vector3 minFontSize = new Vector3(1, 1, 1);
        public Vector3 maxFontSize = new Vector3(10, 10, 10);

        //for grid layout auto font size
        public float sizeGraceAmount = 0.5f;

        public float fontSizeIterationAmount = 0.25f;
        public int maxIterationTimes = 1000;
        ...
        private void CreateSingleMesh()
        {
            ...
            LinearLayoutGroup linearLayoutGroup = GetComponent<LinearLayoutGroup>();
            GridLayoutGroup gridLayoutGroup = GetComponent<GridLayoutGroup>();

            if (autoFontSize && linearLayoutGroup)
            {
                meshLayouts = linearLayoutGroup.GetPositions(meshLayouts, out float totalSpaceTaken, true);
                float multiplier = linearLayoutGroup.width / totalSpaceTaken;
                Vector3 correctedFontSize = GetChildSize(multiplier);
                _fontSize = correctedFontSize;
                ...
                {
                    meshLayouts[i].width *= widthChangeRate;
                }

                meshLayouts = linearLayoutGroup.GetPositions(meshLayouts, linearLayoutGroup.spacing * multiplier);
                combinedMeshes = MeshCombiner.CombinedMesh(meshLayouts, correctedFontSize, useIncreasedVertexCountForCombinedMesh);
            }
            else if (autoFontSize && gridLayoutGroup)
            {
                meshLayouts = gridLayoutGroup.GetPositions(meshLayouts, out float totalVerticalSpaceTaken, out float totalHorizontalSpaceTaken);
                ...
                Vector3 newFontSize = FontSize;
                float widthChangeRate = 1;
                float heightChangeRate = 1;
                int loopBreaker = 0;
                while (tooBig && loopBreaker < maxIterationTimes)
                {
                    loopBreaker++;
                    ...
                    for (int i = 0; i < meshLayouts.Count; i++)
                    {
                        meshLayouts[i].xOffset *= widthChangeRate;
                        meshLayouts[i].width *= widthChangeRate;
                        meshLayouts[i].height *= heightChangeRate;
                    }
                    _fontSize = newFontSize;

                    meshLayouts = gridLayoutGroup.GetPositions(meshLayouts, out totalVerticalSpaceTaken, out totalHorizontalSpaceTaken);
                ...
                //    Vector3 center = gridLayoutGroup.bounds[i].center;
                //    center.x *= widthChangeRate;
                //    center.y *= heightChangeRate;
                //    gridLayoutGroup.bounds[i].center = center;
                //}
            ...
            else
            {
                ...
                combinedMeshes = MeshCombiner.CombinedMesh(meshLayouts, FontSize, useIncreasedVertexCountForCombinedMesh);
            }
        ...
        private int NewCharacterStartsFrom()
        {
            ...
            //Auto Size and not combined mesh has a bug with linear layout updating incorrectly. This is a temporary fix
            if (autoFontSize && (!combineMeshInEditor || !combineMeshDuringRuntime) && GetComponent<LinearLayoutGroup>())
                return 0;
        ...
        private void DestroyObjectRuntime(GameObject obj)
        {
            ...
            else
            {
                if (combineMeshDuringRuntime)
                {
                    if (obj.GetComponent<MeshFilter>() != null)
                        if (obj.GetComponent<MeshFilter>().sharedMesh != null)
                            DestroyMesh(obj.GetComponent<MeshFilter>().sharedMesh);
                }
                else
                {
                    if (obj.GetComponent<MeshFilter>() != null)
        ...
        private void ApplyStyle(GameObject obj)
        {
            ...
            obj.transform.localScale = FontSize;
        ...
        //private Vector3 GetChildSize() => new Vector3(FontSize.x , FontSize.y , FontSize.z);
        private Vector3 GetChildSize(float multiplier)
        {
            return new Vector3(
                Mathf.Clamp(FontSize.x * multiplier, minFontSize.x, maxFontSize.x),
                Mathf.Clamp(FontSize.y * multiplier, minFontSize.y, maxFontSize.y),
                Mathf.Clamp(FontSize.z * multiplier, minFontSize.z, maxFontSize.z));
        }
        ...
        /// <summary>
        /// This is used by a static method to destroy meshes
        /// </summary>
