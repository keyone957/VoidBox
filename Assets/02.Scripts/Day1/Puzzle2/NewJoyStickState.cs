...
public class NewJoyStickState : MonoBehaviour
{
    [SerializeField] Transform joyStickTransform;
       ...
       [SerializeField] MeshRenderer[] pointsMesh;
       private void Awake()
       {
       ...
       private void SetDiskIndex()
       {
           ...
           for (int i = 0; i < pointsMesh.Length; i++)
           {
               pointsMesh[i].material = (i == _diskRotater.diskIndex) ? enableMat : disableMat;
           }
       }
   
       public void ChangeImg(string mode)
       {
           if ((int)DayOneManager.Instance.CurState < (int)DayOneManager.GameState.PUZZLE3) return;
           if (mode == "prev"&&currentImageIndex > 0)
           {
               currentImageIndex--;
               screenImg.texture = img[currentImageIndex];
           }
           else if(mode=="next"&&currentImageIndex < img.Count - 1)
           {
               currentImageIndex++;
               screenImg.texture = img[currentImageIndex];
           }
