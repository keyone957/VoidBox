...
public class CoverFSM : MonoBehaviour
{
    [Header("Pursuit")]
    [SerializeField]
    ...
    [SerializeField]
    private GameObject coverProjectilePrefab;
    [SerializeField]
    private Transform coverProjectileSpawnPoint;
    [SerializeField]
    ...
    ///////////////////////////////
    public void SetUp3(Transform target, Transform coverPos)
    {
...
////////////////////////////////
    public void Setup(Transform target, Transform coverPos, EnemyMemoryPool enemyMemoryPool)
    {
        status = GetComponent<Status>();
        navMeshAgent = GetComponent<NavMeshAgent>();
        this.target = target;
    ...
    public void SetUp2(Transform target, Transform coverPos,NightGameSystem nightGameSystem)
    {
        status = GetComponent<Status>();
        navMeshAgent = GetComponent<NavMeshAgent>();
        this.target = target;
    ...
    private IEnumerator Pursuit()
    {
        while (true)
        {
            ...
            navMeshAgent.SetDestination(coverPos.position);
    ...
    //             StartCoroutine(RayAndShoot(ray));
    //         }
    //         yield return null;
    //     }
    // }
    private IEnumerator Attack()
    {
        ...
        Transform shootPos = coverPos.GetChild(child);
        lastAttackTime = Time.time;
        while (true)
        {
            navMeshAgent.speed = status.RunSpeed;
            navMeshAgent.SetDestination(shootPos.position);

            LookRotationToTarget(target);
            if (Time.time - lastAttackTime > attackRate)
            {
    ...
    IEnumerator RayAndShoot(GameObject ray)
    {
        ...
        while (elapsedTime < rayRate)
        {
            if (lineRenderer != null && target != null)
            {
                ...
                lineRenderer.SetPosition(0, coverProjectileSpawnPoint.position);
                lineRenderer.SetPosition(1, target.position); // TODO Modify Position - new Vector3(0, -0.8f, 0));
            }
    ...
    public void OnAttack() // Called in animation event
    {
        ...
        if (coverProjectilePrefab != null)
        {
            // Calculate the direction from the spawn point to the target
            Vector3 direction = (targetPosition - coverProjectileSpawnPoint.position).normalized;

            // Shoot a ray in the calculated direction
            Ray ray = new Ray(coverProjectileSpawnPoint.position, direction);
            RaycastHit hit;
            ...
            GameObject clone = Instantiate(coverProjectilePrefab, coverProjectileSpawnPoint.position, rotation);
        }
        else
        {
    ...
    private void LookRotationToTarget(Transform lookPoint)
    {
        /*
        Vector3 to = new Vector3(coverPos.position.x, 0, coverPos.position.z);
        Vector3 from = new Vector3(transform.position.x, 0 , transform.position.z);
    ...
    private void CalculateDistanceTOTargetAndSelectState()
    {
        ...
        float distance = Vector3.Distance(coverPos.position, transform.position);
