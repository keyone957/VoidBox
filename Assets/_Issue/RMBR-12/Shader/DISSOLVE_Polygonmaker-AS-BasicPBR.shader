...
Shader "Polygonmaker/Dissolve-Basic-PBR"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				// WOOKY
				// Sample the noise texture for dissolve
				float noise = tex2D(_NoiseTex, uv_MainTex).r;

				// Calculate dissolve mask and edge
				float edge = smoothstep(_DissolveThreshold - _EdgeWidth, _DissolveThreshold, noise);
				float mask = step(_DissolveThreshold, noise);
			
				// Apply edge effect
				half4 edgeEffect = _EdgeColor * (1.0 - edge);
			
				// Mix edge effect and base color
				color.rgb = lerp(color.rgb, edgeEffect.rgb, edge);
			
				// Apply the dissolve mask to alpha
				color.a = 1.0;
			
				// Ensure transparency blending is handled correctly
				#ifdef _ALPHATEST_ON
					clip(color.a - 0.1); // Adjust threshold as needed
				#endif
